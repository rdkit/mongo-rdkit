...
def test_addPatternFingerprints():
    ...
    substructure.AddPatternFingerprints(db)
    counter = 0
...
def test_SubSearchAccuracy():
    ...
    write.writeFromSDF(db_mock, 'data/test_data/first_200.props.sdf', 'test')
    substructure.AddPatternFingerprints(db_mock)
    ...
    for i in range(200):
        ...
        results = substructure.SubSearch(pattern, db_mock)
